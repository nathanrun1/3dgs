...
float bivariate_pdf(vec2 x, mat2 cov) {
    ...
    float z2 = dot(x, inv_cov * x);
    float pdf = (1 / sqrt(2*PI)) * exp(-0.5 * z2);
    return (1 / sqrt(determinant(cov))) * pdf;
}

float unnormalized_bivariate_pdf(vec2 x, mat2 cov) {
    return (1 / sqrt(2*PI)) * exp(-0.5 * dot(x, inverse(cov) * x));
}
